 ...
 * per-page .md route — deliberately separate from SearchDocument, which
 * strips markup for tokenization. AI consumers want the real source
 * (headings, links, code fences intact), not search-stripped plain text.
 */
 ...
 * backs llms.txt and llms-full.txt. Same iteration shape as
 * getAllSearchDocuments, deliberately not sharing an implementation with
 * it: that function strips markup for search tokenization, this one must
 * not (AI consumers want the real source, headings/links/code intact).
 */
