...
SVOCS also supports Orama, FlexSearch, Typesense, and Chroma behind the exact same `⌘K` dialog — none of them are bundled into this starter by default, to keep a fresh install small. Pick one, install the one package it needs, and follow the setup guide:
